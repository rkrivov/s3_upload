...
class Urn(object):
    ...
    def compare_path(path_a, href):
        unqouted_path = unquote(urlsplit(href).path)
        if not unqouted_path.startswith(Urn.separate):
            unqouted_path = Urn.separate + unqouted_path

        if not path_a.startswith(Urn.separate):
            path_a = Urn.separate + path_a

        path_a = Urn.normalize_path(path_a)
        unqouted_path = Urn.normalize_path(unqouted_path)

        return True if path_a == unqouted_path else False

# u'/Backups/Новый текстовый документ.txt'
# u'/Backups/Новый текстовый документ.txt'
