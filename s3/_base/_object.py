...
from common.consts import BUFFER_SIZE
from common.utils import total_len
...
class S3Object(object):
    def __init__(self, name: Union[str, bytes], response: Union[Dict, StreamingBody]):
        ...
        if hasattr(self._body, '_raw_stream'):
            self._buffer_reader = io.BufferedReader(self._body._raw_stream, buffer_size=BUFFER_SIZE)
        else:
    ...
    def read(self, chuck_size=BUFFER_SIZE):
        ...
        self._tell += total_len(chuck)

        return chuck

    def next(self, chuck_size=BUFFER_SIZE):
