...
class S3Base(object):
    ...
    def __init__(self, bucket: str, local_path: Optional[str] = None, remote_path: Optional[str] = None):
        ...
        logger.debug(f"Chuck size is {utils.size_to_human(self._chuck_size)}")
    ...
    def bucket_hash(self):
        return utils.encode_string(self.bucket_id)
    ...
    def _calc_hash(self, file_path: str) -> str:
        ...
        if file_hash is None:
            templates = {
                'LOCAL_PATH': utils.remove_end_path_sep(self._local_path),
                'ARCHIVE_PATH': utils.remove_end_path_sep(self._archive_path),
                'CONTAINER': utils.remove_end_path_sep(consts.CONTAINERS_FOLDER),
                'LIBRARY': utils.remove_end_path_sep(consts.LIB_FOLDER),
                'HOME': utils.remove_end_path_sep(consts.HOME_FOLDER),
                'WORK': utils.remove_end_path_sep(consts.WORK_FOLDER),
                'TEMP': utils.remove_end_path_sep(consts.TEMP_FOLDER),
            }

            logger.debug(
                f"Calculate hash {consts.MD5_ENCODER_NAME.upper()} "
                f"for {utils.make_template_from_string(file_path, **templates)}"
            )

            # file_hash = utils.get_file_etag(file_name=file_path, show_progress=self._show_progress)
            # file_hash_2 = utils.calc_file_hash(file_object=file_path, show_progress=self._show_progress)
            if utils.get_file_size(file_name=file_path) > consts.FILE_SIZE_LIMIT:
                file_hash = utils.get_file_etag(file_name=file_path, show_progress=self._show_progress)
            else:
                file_hash = utils.calc_file_hash(file_object=file_path, show_progress=self._show_progress)
        ...
        if file_info_new is not None or file_info_old is not None:
            ...
            if file_info_old is not None and file_info_new is not None:
                ...
                messages.append(
                    ...
                    f"({consts.CYAN + consts.BOLD}{utils.size_to_human(file_size_old)}{consts.NBOLD + consts.DEF})"
                    " differs from the size of the local file "
                    f"({consts.CYAN + consts.BOLD}{utils.size_to_human(file_size_new)}{consts.NBOLD + consts.DEF})"
                )
    ...
    def _copy_object(self, src: str, dst: str) -> None:
        ...
        if self._show_progress:
            ...
            progress = utils.ProgressBar(caption="Copying objects", max_value=objects_count)
            ...
            for fetch_object in self.storage.fetch_bucket_objects(prefix=src):
                ...
                if dst_name.startswith(src):
                    dst_name = os.path.join(
                        utils.append_end_path_sep(dst),
                        utils.remove_start_separator(dst_name[len(src):])
                    )
    ...
    def _fetch_local_files_list(self, local_path: str = None, opetations: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if objects_count > 0:
            ...
            try:
                for remote_object in self.storage.fetch_bucket_objects(prefix=prefix):
                    ...
                    file_name = utils.convert_value_to_type(remote_object.get('Key', None), to_type=str)
    ...
    def _make_local_file(self, file_name: str) -> str:
        return utils.make_string_from_template(file_name, path=utils.remove_end_path_sep(self._local_path))

    def _make_remote_file(self, file_name: str) -> str:
        return utils.make_string_from_template(file_name, path=utils.remove_end_path_sep(self._archive_path))
    ...
    def _show_statistics(self):
        ...
        if self._inserted_new_files > 0:
            statistics.append(
                ...
                f'{utils.get_string_case(self._inserted_new_files, "file", "operations")} was inserted'
            )

        if self._updated_files > 0:
            statistics.append(
                f'{self._updated_files} '
                f'{utils.get_string_case(self._updated_files, "file", "operations")} was updated'
            )

        if self._deleted_files > 0:
            statistics.append(
                f'{self._deleted_files} '
                f'{utils.get_string_case(self._deleted_files, "file", "operations")} was deleted'
            )
    ...
    def set_archive_path(self, remote_path: str):
        ...
        archive_name = utils.remove_end_path_sep(remote_path)
        ...
        self._archive_path = utils.remove_end_path_sep(remote_path)
        self._bak_archive_path = utils.remove_end_path_sep(archive_path)
